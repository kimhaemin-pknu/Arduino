...
void loop() {
  ...
  if (buttonState1 == LOW) {
    ...
    if (millis() - buttonPressTime1 <= 2000) {  // 2초 이하 눌렀을 때
      // 알람 주기 30분 증가
      if (!ledOn) { // 알람 LED가 켜져 있을 때 30분 추가 안됨
        alarmTime += 30;
        Serial.print("30분 증가: ");
...
void setRTCAlarm() {
  ...
  int alarmMinutes = now.minute() + alarmTime; // 현재 시간 기준으로 설정된 알람 시간 추가
  ...
  rtc.setAlarm1(
    DateTime(now.year(), now.month(), now.day(), alarmHours, alarmMinutes, now.second()),
    DS3231_A1_Minute
  );
